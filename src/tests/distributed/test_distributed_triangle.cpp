...
// =============================================================================
//
// Test triangle collision shape in Chrono::Distributed
//
...
void AddTriangle(ChSystemDistributed* sys, double height) {
    ChVector<> inertia = 10.0 * (2.0 / 5.0) * mass * gran_radius * gran_radius * ChVector<>(1, 1, 1);

    auto triangle = std::make_shared<ChBody>(std::make_shared<ChCollisionModelDistributed>(), ChMaterialSurface::SMC);
    auto trimat = std::make_shared<ChMaterialSurfaceSMC>();
    trimat->SetYoungModulus(Y);
    trimat->SetFriction(mu);
    trimat->SetRestitution(cr);
    trimat->SetAdhesion(0);
    triangle->SetMaterialSurface(trimat);

    triangle->SetMass(mass);  // TODO
    triangle->SetInertiaXX(inertia);
    triangle->SetPos(ChVector<>(0, 0, height + 2.0 * gran_radius));
    triangle->SetRot(ChQuaternion<>(1, 0, 0, 0));
    triangle->SetBodyFixed(false);
    triangle->SetCollide(true);

    triangle->GetCollisionModel()->ClearModel();
    ChVector<> A(-5.0 * gran_radius, -5.0 * gran_radius, 0);
    ChVector<> B(5.0 * gran_radius, -5.0 * gran_radius, 0);
    ChVector<> C(0, 5.0 * gran_radius, 0);
    std::dynamic_pointer_cast<ChCollisionModelDistributed>(triangle->GetCollisionModel())
        ->AddTriangle(A, B, C, ChVector<>(0), ChMatrix33<>(1));
    triangle->GetCollisionModel()->BuildModel();
    sys->AddBody(triangle);
    sys->IncrementGID();
}
...
int main(int argc, char* argv[]) {
    ...
    ChVector<double> domlo(-h_x, -h_y, -0.0001);
    ChVector<double> domhi(h_x, h_y, height + 0.0001);
